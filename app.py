...
def home():
    ...
    if flask.request.method == 'POST':
        image1 = flask.request.files.get('img1')
        image2 =flask.request.files.get('img2')
        image =flask.request.files.get('img')    
        return render_template('image-image.html', string = 'PHOTOS RECEIVED AND PROCESSING')
    
@app.route("/Stego")
def cli():
    print(" Helloo Worlldd!")
